  ...
  return (
    <div className="min-h-screen bg-zinc-950 px-4 py-8 max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-zinc-100">My Groups</h1>
          <p className="text-zinc-500 text-sm mt-0.5">Coordinate with your crew</p>
        </div>
        <button
          onClick={openModal}
          className="bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold text-sm rounded-xl px-4 py-2.5 transition-colors flex items-center gap-1.5"
        >
      ...
      {!isLoading && !error && groups.length === 0 && (
        <div className="text-center py-20">
          ...
          <p className="text-zinc-200 font-semibold mb-1.5">No groups yet</p>
      ...
      {modalOpen && (
        ...
        >
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-sm">
            <h2 className="text-zinc-100 font-bold text-lg mb-1">Create group</h2>
            <p className="text-zinc-500 text-sm mb-5">Give your crew a name.</p>
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label htmlFor="group-name" className="block text-sm font-medium text-zinc-300 mb-1.5">
                  Group name
                </label>
                <input
                  id="group-name"
              ...
              <div className="flex gap-3 pt-1">
                <button
                  type="button"
                  onClick={closeModal}
                  className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 font-medium text-sm rounded-xl py-2.5 transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 disabled:cursor-not-allowed text-zinc-950 font-bold text-sm rounded-xl py-2.5 transition-colors"
                >
